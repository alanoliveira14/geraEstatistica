...
Public Class definirEscalacaoAdv
    Dim cn As SqlConnection
    Dim cmd As SqlCommand
    Dim cmd1 As SqlCommand
    Dim mr As SqlDataReader
    Public a As String = "select * from jogador where codClube = " & (CadPartida.cmbTime2.SelectedValue).ToString
    Dim b As String = "insert into relacionadosAdv (codEscalacaoAdv, codJogador, situacao) values "
    Dim t As Integer = 0
    ...
    Private Sub btnAddTitular_Click() Handles btnAddTitular.Click
        ...
        If t = 0 Then
            Using cn As SqlConnection = conexao()
                ...
                If b = "insert into relacionadosAdv (codEscalacaoAdv, codJogador, situacao) values " Then
                    b = b + "((select MAX(codEscalacaoAdv) from escalacaoAdv)," & lstJogadores.SelectedValue & ", 'Titular')"
                Else
                    b = b + ",((select MAX(codEscalacaoAdv) from escalacaoAdv)," & lstJogadores.SelectedValue & ", 'Titular')"
                End If
        ...
        Else
            Using cn As SqlConnection = conexao()
                ...
                a = a + " and codJogador != " & Me.lstJogadores.SelectedValue
                If b = "insert into relacionadosAdv (codEscalacaoAdv, codJogador, situacao) values " Then
                    b = b + "((select MAX(codEscalacaoAdv) from escalacaoAdv)," & lstJogadores.SelectedValue & ", 'Reserva')"
                Else
                    b = b + ",((select MAX(codEscalacaoAdv) from escalacaoAdv)," & lstJogadores.SelectedValue & ", 'Reserva')"
                End If
                Try
    ...
    Public Sub insertEscalacao()
        If Me.lstTitulares.Items.Count = 11 And Me.lstSuplentes.Items.Count >= 5 Then
            Using cn As SqlConnection = conexao()
                Dim queryescalacao As String = "insert into escalacaoAdv values (" & (cmbEsqTatico.SelectedValue).ToString & "," & (CadPartida.cmbTime2.SelectedValue).ToString & ", (select MAX(codPartida) from partida))"
    ...
    Public Function carregaImagem()
        Using cn As SqlConnection = conexao()
            cn.Open()
            Dim sql As String = "select imagem from timeAdversario where codTimeAdversario = " & (CadPartida.cmbTime2.SelectedValue).ToString & ""
            ...
            cn.Open()
            Dim sql2 As String = "select alcunha from timeAdversario where codTimeAdversario = " & CadPartida.cmbTime2.SelectedValue & ""
            cmd = New SqlCommand(sql2, cn)
            da = New SqlDataAdapter(cmd)
            Dim x As String
            x = cmd.ExecuteScalar
            Me.lblNomeTimeAdv.Text = x
            cn.Close()
        End Using
        Return Nothing
    End Function
    ...
    Private Sub removerJogador()
        ...
        End If
        a = "select * from jogador where codClube = " & (CadPartida.cmbTime2.SelectedValue).ToString
        b = "insert into relacionadosAdv (codEscalacaoAdv, codJogador, situacao) values "
        For g = 0 To Me.lstAux.Items.Count - 1
            Me.lstAux.SetSelected(g, 1)
            a = a + " and codJogador != " & Me.lstAux.Text
            If g = 0 Then
                b = b + "((select MAX(codEscalacaoAdv) from escalacaoAdv)," & Me.lstAux.Text & ", 'Titular')"
            Else
                b = b + ",((select MAX(codEscalacaoAdv) from escalacaoAdv)," & Me.lstAux.Text & ", 'Titular')"
            End If
        Next
        ...
        For f = 0 To Me.lstAux2.Items.Count - 1
            ...
            b = b + ",((select MAX(codEscalacaoAdv) from escalacaoAdv)," & Me.lstAux2.Text & ", 'Reserva')"
        Next
        carregaJogadores()
    End Sub
End Class